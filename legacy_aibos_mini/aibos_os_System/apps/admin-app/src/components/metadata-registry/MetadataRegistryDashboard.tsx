...
export default function MetadataRegistryDashboard({
  ...
  const loadData = async () => {
    ...
    try {
      switch (activeTab) {
        case 'registry':
          const fields = await metadataService.searchMetadataFields(searchQuery, {
        ...
        case 'local':
          ...
          break;
        case 'governance':
          const metrics = await metadataService.getGovernanceMetrics(organizationId);
          setGovernanceMetrics(metrics);
          break;
        case 'dictionary':
          const dictionary = await metadataService.generateDataDictionary(organizationId);
