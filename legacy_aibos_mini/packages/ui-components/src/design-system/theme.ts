// AI-BOS OS Design System Theme
// Modern, accessible, and scalable design tokens
...
export const getColor = (color: string, scale?: ColorScale) => {
  if (scale) {
    return theme.colors[color as keyof typeof theme.colors]?.[scale];
  }
  return theme.colors[color as keyof typeof theme.colors];
};

export const getSpacing = (size: Spacing) => theme.spacing[size];
export const getBorderRadius = (size: BorderRadius) => theme.borderRadius[size];
export const getShadow = (size: Shadow) => theme.shadows[size];
export const getTransition = (speed: Transition) => theme.transitions[speed];
export const getZIndex = (level: ZIndex) => theme.zIndex[level]; 