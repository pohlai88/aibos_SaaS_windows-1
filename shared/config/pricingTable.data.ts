...
export function getPlanByMetadata(table: EnhancedPricingTable, plan: SubscriptionPlan, interval: BillingInterval): EnhancedPricingTableEntry | undefined {
  return table.find(entry => entry.plan === plan && entry.interval === interval);
}

export function getPlansWithContactButton(table: EnhancedPricingTable): EnhancedPricingTableEntry[] {
...
export function getPlansWithDiscount(table: EnhancedPricingTable): EnhancedPricingTableEntry[] {
  return table.filter(entry => entry.metadata.annualDiscount && entry.metadata.annualDiscount > 0);
}

export function getMostPopularPlan(table: EnhancedPricingTable): EnhancedPricingTableEntry | undefined {
