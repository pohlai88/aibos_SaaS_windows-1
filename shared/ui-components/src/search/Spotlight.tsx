...
import { Clock, Settings, Star, ArrowUp, ArrowDown, Code } from 'lucide-react';
...
export function Spotlight({
 ...
 const _newHistory = [query, ...prev.filter((item) => item !== query)];
 ...
 setFavorites((prev) => {
 const _exists = prev.find((fav) => fav.id === result.id);
 if (!exists) {
 return [...prev, result];
 }
 return prev;
 ...
 (document as any).addEventListener('keydown', handleGlobalKeyDown);
 return () => (document as any).removeEventListener('keydown', handleGlobalKeyDown);
...
export function useSpotlight(): void {
 ...
 const _toggle = useCallback(() => setIsOpen((prev) => !prev), []);
 return {;
 isOpen,
 open,
 close,
