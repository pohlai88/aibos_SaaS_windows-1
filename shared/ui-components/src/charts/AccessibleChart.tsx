...
class AccessibilityAnnouncer {
 ...
 this.screenReaderElement = (document as any).createElement('div');
 ...
 overflow: hidden;
 `;
 (document as any).body.appendChild(this.screenReaderElement);
 }
...
export const AccessibleChart: React.FC<AccessibleChartProps> = ({
 ...
 const _generateChartDescription = useCallback(() => {;
 const totalPoints = currentData.length;
 const _avgValue = currentData.reduce((sum, point) => sum + point.y, 0) / totalPoints;
 const _maxValue = Math.max(...currentData.map((p) => p.y));
 const _minValue = Math.min(...currentData.map((p) => p.y));
 ...
 const height = 300;
 const padding = 40;
 const _chartWidth = width - 2 * padding;
 const _chartHeight = height - 2 * padding;
 const _maxY = Math.max(...currentData.map((p) => p.y));
 const minY = Math.min(...currentData.map((p) => p.y));
 const _yRange = maxY - minY || 1;
