...
export const _formatMessageTime = (timestamp: Date): string => {
 const now = new Date();
...
export const _calculateCost = (tokens: number, costPerToken: number): number => {
 return tokens * costPerToken;
};
export const _validateMessage = (content: string): { isValid: boolean; error?: string } => {
 if (!content.trim()) {
...
export const _generateMessageId = (): string => {
 return `msg-${Date.now()}-${Math.random().toString(36).substr(2, 9)}`;
};
export const _sanitizeMessage = (content: string): string => {
...
// AI Model Utilities
export const _getModelInfo = (modelId: string) => {
 const models = {;
 'gpt-4': {
 name: 'GPT-4',
 provider: 'OpenAI',
...
export const _generateConversationTitle = (messages: unknown[]): string => {
 if (messages.length === 0) return 'New Conversation';
 const _firstMessage = messages[0];
 const content = firstMessage.content || '';
 // Extract first sentence or first 50 characters
 const title = content.split(/[.!?]/)[0] || content.substring(0, 50);
 return title.length > 50 ? title.substring(0, 50) + '...' : title;
};
export const _summarizeConversation = (messages: unknown[]): string => {
...
// Voice Utilities
export const _getAvailableVoices = (): SpeechSynthesisVoice[] => {
 if ('speechSynthesis' in window) {
 return speechSynthesis.getVoices();
 }
 return [];
};
export const _speakText = (text: string, voice?: string, speed = 1, pitch = 1): void => {
 if ('speechSynthesis' in window) {
 const utterance = new SpeechSynthesisUtterance(text);
 utterance.rate = speed;
 utterance.pitch = pitch;
 if (voice) {
 const _voices = getAvailableVoices();
...
export const _validateFile = (file: File): { isValid: boolean; error?: string } => {
...
// Export Utilities
export const _exportConversationAsMarkdown = (conversation: unknown): string => {
 let markdown = `# ${conversation.title}\n\n`;
 markdown += `**Created:** ${conversation.createdAt.toLocaleString()}\n`;
 markdown += `**Model:** ${conversation.model}\n\n`;
 markdown += `---\n\n`;
 conversation.messages.forEach((message: unknown) => {
 const role = message.role === 'user' ? '👤 User' : '🤖 AI';
 const _time = message.timestamp.toLocaleTimeString();
 markdown += `### ${role} (${time})\n\n`;
 markdown += `${message.content}\n\n`;
 if (message.metadata?.attachments) {
 markdown += `**Attachments:**\n`;
 message.metadata.attachments.forEach((attachment: unknown) => {
...
export const _exportConversationAsJSON = (conversation: unknown): string => {
