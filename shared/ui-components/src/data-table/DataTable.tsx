...
const _DataTable = <T,>({;
 ...
 const _getRowClasses = (row: T, index: number) => {;
 ...
 const _clickableClasses = onRowClick ? 'cursor-pointer hover:bg-blue-50' : '';
 const _customClasses = getRowClassName ? getRowClassName(row, index) : '';
 return `${baseClasses} ${variantClasses[variant]} ${clickableClasses} ${customClasses}`;
 };
 const _handleRowClick = (row: T, index: number) => {;
 if (onRowClick) {
 ...
 const _startItem = (page - 1) * pageSize + 1;
 const _endItem = Math.min(page * pageSize, total);
 return (;
 ...
 const _isSelected = selection?.selectedRows?.includes(rowKey) || false;
