...
export const ExcelLikeGrid: React.FC<ExcelLikeGridProps> = ({
  ...
  const parseFormula = useCallback((formula: string, row: number, col: number): string => {
  ...
  const updateCell = useCallback((row: number, col: number, value: string, formula?: string) => {
    setGridData(prev => {
      ...
      newData[row][col] = {
        ...
        formula,
        format: columns[col]?.format as any || 'text',
      };
    ...
    clipboard.forEach((rowData, rowOffset) => {
      ...
      rowData.forEach((cellValue, colOffset) => {
        ...
        newData[targetRow][targetCol] = {
          ...
          format: columns[targetCol]?.format as any || 'text',
        };
      });
    });
  ...
  const renderCell = useCallback((row: number, col: number) => {
    ...
    selection, editingCell, getCellValue, gridData, columns, variant,
    handleCellClick, handleCellDoubleClick, handleKeyDown, formulaInput,
    handleFormulaSubmit
  ]);
    ...
    <div className={cn(gridVariants({ variant, size }), className)}>
      ...
      <div className="border-t border-border p-2 bg-muted/20 text-xs text-muted-foreground">
        <div className="flex items-center justify-between">
          ...
          <span>
            {clipboard.length > 0 && `Clipboard: ${clipboard.length} row${clipboard.length !== 1 ? 's' : ''}`}
          </span>
        </div>
      </div>
    </div>
