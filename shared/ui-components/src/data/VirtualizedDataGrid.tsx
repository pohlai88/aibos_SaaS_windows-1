...
const _usePerformanceMonitor = (componentName: string) => {;
...
export const VirtualizedDataGrid = <T extends Record<string, any>>({
 ...
 const _fps = usePerformanceMonitor('VirtualizedDataGrid');
 ...
 const _updatesPerSecond = 1000;
 const _updatesPerInterval = Math.floor(updatesPerSecond * (realTimeInterval / 1000));
...
export const VirtualizedDataGridTest: React.FC = () => {
 ...
 key: 'email',
 header: 'Email',
 accessor: (row: unknown) => row.email,
 width: 250
 },
 {
 key: 'status',
 header: 'Status',
 accessor: (row: unknown) => row.status,
 width: 120,
 ...
 key: 'value',
 header: 'Value',
 accessor: (row: unknown) => row.value,
 width: 120,
 cellRenderer: (value: unknown) => <span className="font-mono">${value.toFixed(2)}</span>
 },
 {
 key: 'actions',
 header: 'Actions',
 accessor: (row: unknown) => row,
 width: 150,
 expandable: true,
 nestedRenderer: (row: unknown) => (
