...
import { CheckCircle, Clock, Settings, TrendingUp, AlertTriangle, Star, Zap, Trash, Circle } from 'lucide-react';
...
export const _DataGrid = <T extends Record<string, any>>({
 data,
 columns,
 loading = false,
 error = null,
 ...
 const _hasDates = sampleValues.some((v) => v instanceof Date);
 const _hasNumbers = sampleValues.some((v) => typeof v === 'number');
 ...
 const _getPredictiveFilters = useCallback(() => {;
 ...
 const _frequentlyUsed = Object.entries(usageStats);
 ...
 const _VirtualizedRows = useCallback(() => {;
 ...
 const _endIndex = Math.min(data.length, startIndex + visibleCount + overscan * 2);
 const _visibleData = data.slice(startIndex, endIndex);
 const _totalHeight = data.length * itemHeight;
 const _offsetY = startIndex * itemHeight;
 return (;
 <div
 ...
 const _exportData = selection?.selectedRows.size;
 ...
 const _searchLower = globalSearch.toLowerCase();
 ...
 const _startWidth = columnWidths[column.key] || column.width || 200;
 const handleMouseMove = (e: MouseEvent) => {;
 const _deltaX = e.clientX - startX;
