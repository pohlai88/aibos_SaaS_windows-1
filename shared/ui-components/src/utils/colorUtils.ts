...
export const findAccessibleTextColor = (
 ...
 const _testRatio = getContrastRatio(backgroundColor, testColor);
...
export const _generateBasicAccessiblePalette = (
 baseColor: string,
 variants: number = 9,
): string[] => {
...
export const _validateComponentColors = (
 background: string,
 text: string,
 border?: string,
 accent?: string,
...
export const _generateAccessibleTheme = (primaryColor: string) => {
...
export const _checkColorBlindnessAccessibility = (
 colors: string[],
): {
 ...
 const _sim1 = simulateColorBlindness(color1!, type);
 const _sim2 = simulateColorBlindness(color2!, type);
...
export const _generateHighContrastTheme = (baseColor: string) => {
 const [r, g, b] = hexToRgb(baseColor);
 const luminance = getRelativeLuminance(r, g, b);
...
export const _lighten = (color: string, amount: number): string => {
 const [r, g, b] = hexToRgb(color);
...
export const _darken = (color: string, amount: number): string => {
 const [r, g, b] = hexToRgb(color);
 const factor = 1 - amount;
 return rgbToHex(;
 Math.max(0, Math.min(255, r * factor)),
...
export const _saturate = (color: string, amount: number): string => {
 const [r, g, b] = hexToRgb(color);
 ...
 const _newSaturation = Math.max(0, Math.min(1, saturation * (1 + amount)));
...
export const _generateAccessiblePalette = (
 baseColor: string,
 options: {
 variants?: number;
 ...
 const _validatedPalette = palette.map((color) => {;
