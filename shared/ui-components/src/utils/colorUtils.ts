...
export const validateComponentColors = (
  ...
  if (!textContrast.passes.AA) {
    issues.push(`Text contrast ratio ${textContrast.ratio.toFixed(2)} is below WCAG AA standard (4.5)`);
  ...
  if (border) {
    ...
    if (!borderContrast.passes.AA) {
      issues.push(`Border contrast ratio ${borderContrast.ratio.toFixed(2)} is below WCAG AA standard (4.5)`);
  ...
  if (accent) {
    ...
    if (!accentContrast.passes.AA) {
      issues.push(`Accent contrast ratio ${accentContrast.ratio.toFixed(2)} is below WCAG AA standard (4.5)`);
...
export const checkColorBlindnessAccessibility = (colors: string[]): {
  ...
  for (let i = 0; i < colors.length; i++) {
    for (let j = i + 1; j < colors.length; j++) {
      ...
      for (const type of types) {
        ...
        if (contrast < 2.0) {
          issues.push(`Colors ${color1} and ${color2} are not distinguishable for ${type} users (contrast: ${contrast.toFixed(2)})`);
          suggestions.push(`Consider using colors with higher contrast or adding patterns/icons to distinguish between ${color1} and ${color2}`);
        }
      }
    }
  }
