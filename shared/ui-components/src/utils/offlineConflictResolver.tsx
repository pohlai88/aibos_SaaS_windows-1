...
export interface ConflictResolutionProps extends VariantProps<typeof conflictVariants> {
  ...
  onResolve: (strategy: ConflictStrategy, data?: any) => void;
  className?: string;
}
...
export const ConflictResolution: React.FC<ConflictResolutionProps> = ({
  ...
  const renderDataComparison = (field: string) => {
...
export const OfflineConflictTest: React.FC = () => {
  ...
  const simulateOfflineAction = () => {
    ...
    console.log('Added offline action:', actionId);
  };

  const resolveConflict = (actionId: string, strategy: ConflictStrategy, data?: any) => {
