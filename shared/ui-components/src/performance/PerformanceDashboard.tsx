...
import { Clock, TrendingUp, TrendingDown, AlertTriangle, Zap, HardDrive, Network } from 'lucide-react';
...
export function PerformanceDashboard({
 ...
 setMetrics((prev) => {
 const _updated = [...prev, newMetrics];
 ...
 const _checkAlerts = useCallback(;
 ...
 // Generate optimization suggestions
 const _generateSuggestions = useCallback((metrics: PerformanceMetrics) => {;
 ...
 useEffect(() => {
 fetchMetrics();
 const _interval = setInterval(fetchMetrics, refreshInterval);
 return () => clearInterval(interval);
 }, [fetchMetrics, refreshInterval]);
 // Get current metrics
 const currentMetrics = metrics[metrics.length - 1];
 // Prepare chart data
 const _chartData = metrics.map((m) => ({;
