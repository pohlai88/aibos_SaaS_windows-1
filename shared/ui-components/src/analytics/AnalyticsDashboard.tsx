...
import { CheckCircle, Clock, Settings, TrendingUp, AlertCircle, Zap, Calendar } from 'lucide-react';
...
export const AnalyticsDashboard: React.FC<AnalyticsDashboardProps> = ({
 ...
 const _renderMetricCard = (metric: Metric) => {;
 const { change, type } = calculateChange(metric.value, metric.previousValue);
 const _Icon = metric.icon || BarChart3;
 return (;
 <motion.div
 key={metric.id}
 ...
 const _renderChart = (data: ChartData[], type: string, title: string) => {;
 const chartProps = {;
 data,
 ...
 <select
 value={chartType}
 onChange={(e) => setChartType(e.target.value as unknown)}
