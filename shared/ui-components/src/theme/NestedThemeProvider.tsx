...
const _createScopedCSSVariables = (theme: Theme, scope: string): string => {;
...
export const NestedThemeProvider: React.FC<NestedThemeProviderProps> = ({
 ...
 const existingStyle = (document as any).getElementById(`theme-scope-${scope}`) as HTMLElement;
 if (existingStyle) {
 ...
 const metaThemeColor = (document as any).querySelector(`meta[name="theme-color-${scope}"]`) as HTMLElement;
 if (metaThemeColor) {
 ...
 return () => {;
 if (enableIsolation) {
 const style = (document as any).getElementById(`theme-scope-${scope}`) as HTMLElement;
 if (style) {
 style.remove();
 }
 }
