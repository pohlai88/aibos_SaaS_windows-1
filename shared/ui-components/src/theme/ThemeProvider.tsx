...
export function ThemeProvider({
 ...
 const _getEffectiveTheme = useCallback(;
 ...
 (theme: Theme) => {
 const _root = (document as any).documentElement;
 // Apply CSS variables
 Object.entries(theme.cssVariables).forEach(([key, value]) => {
 root.style.setProperty(key, value);
 });
 // Update meta theme-color
 const metaThemeColor = (document as any).querySelector('meta[name="theme-color"]') as HTMLElement;
 if (metaThemeColor) {
 metaThemeColor.setAttribute('content', theme.colors.background);
 }
 // Add theme class to body
 (document as any).body.className = (document as any).body.className
 .replace(/theme-\w+/g, '')
 .replace(/\s+/g, ' ')
 .trim();
 (document as any).body.classList.add(`theme-${theme.id}`);
 ...
 const _currentIndex = THEMES.findIndex((t) => t.id === currentThemeId);
 const _nextIndex = (currentIndex + 1) % THEMES.length;
