...
export interface JobQueueContextValue {
 ...
 addJob: (jobConfig: unknown) => Promise<Job>;
...
export const JobQueueProvider: React.FC<JobQueueProviderProps> = ({
 ...
 async (jobConfig: unknown): Promise<Job> => {
 try {
 ...
 const _wsUrl = apiEndpoint.replace('http', 'ws').replace('https', 'wss');
 ...
 const _interval = setInterval(fetchJobs, refreshInterval);
...
export const _useJobQueue = (): JobQueueContextValue => {
