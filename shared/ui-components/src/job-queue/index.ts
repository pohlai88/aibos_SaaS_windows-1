...
export const _JOB_QUEUE_COMPONENTS = {
...
export const _DEFAULT_JOB_QUEUE_CONFIG = {
...
export const _formatJobDuration = (startTime: Date, endTime?: Date): string => {
 const _start = new Date(startTime);
 const _end = endTime ? new Date(endTime) : new Date();
...
export const _getJobStatusColor = (status: string): string => {
 switch (status) {
 case 'pending':
...
export const _getJobPriorityColor = (priority: number): string => {
 switch (priority) {
 case 1:
...
export const _getJobPriorityLabel = (priority: number): string => {
 switch (priority) {
 case 1:
 return 'Low';
 case 2:
