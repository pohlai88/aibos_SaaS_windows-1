...
export function exampleFeatureHandling() {
  ...
  const editButtonShown = handleFeatureByPermission(
    adminUser,
...
export function exampleMultiplePermissions() {
  ...
  const hasAllUserPermissions = hasAllPermissions(
    adminUser,
  ...
  const hasAnyBillingPermission = hasAnyPermission(
    regularUser,
    [Permission.CONTENT_READ, Permission.CONTENT_UPDATE, Permission.CONTENT_DELETE]
  );
