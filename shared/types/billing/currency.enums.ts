...
export interface CurrencyMetadata {
  ...
  isoCode: string;
  country: string;
  region: 'global' | 'southeast-asia' | 'asia' | 'middle-east' | 'africa' | 'latin-america' | 'europe' | 'north-america' | 'oceania';
...
export class CurrencyUtils {
  ...
  static format(value: number, currency: Currency): string {
    ...
    const withThousands = integer.replace(
      /\B(?=(\d{3})+(?!\d))/g, 
      meta.thousandsSeparator
    );

    const formattedNumber = meta.decimalDigits > 0 
      ? `${withThousands}${meta.decimalSeparator}${decimal}`
      : withThousands;
...
console.log('Southeast Asian currencies:', seaCurrencies.map(c => c.isoCode));
